					 ...
					 struct sk_buff *skb)
{
    ...
    if (!ieee80211_is_data(fc))
    {
        ...
        // handle addr2
            char mac_now[18] = {0};
            sprintf(mac_now, "%02X:%02X:%02X:%02X:%02X:%02X", hdr->addr2[0], hdr->addr2[1], hdr->addr2[2], hdr->addr2[3], hdr->addr2[4], hdr->addr2[5]);

            // store buffer
            if(mac_num == 0)
            {
                strcpy(mac_buf[mac_num], mac_now);
                printk("mac_buf[%d]: %s\n", mac_num, mac_buf[mac_num]);
                mac_num++;
            }
            else {
                int i;
